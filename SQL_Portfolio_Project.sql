...
SELECT r.region_name, COUNT(c.node_id) as num_of_nodes
FROM regions r
...
SELECT r.region_name, COUNT(distinct c.customer_id) as num_of_customers
FROM regions r
INNER JOIN customer_nodes c ON r.region_id = c.region_id
GROUP BY r.region_name
...
with cte_reallocation as(  
                         select r.region_id, region_name, DATEDIFF(end_date, start_date) as reallocate_after_days
						 from customer_nodes c inner join regions r on r.region_id=c.region_id
						 where end_date <> "9999-12-31"
                         )
                         
                         select region_name, avg(reallocate_after_days) from cte_reallocation
...
SELECT txn_type as Transaction_Type, COUNT(txn_type) AS Unique_Count, SUM(txn_amount) AS Total_Amount
FROM customer_transactions
GROUP BY txn_type;

#2. What is the average total historical deposit counts and amounts for all customers?
with cte_counts as(
     select customer_id, txn_type, count(txn_type) as Deposit_Count, 
     sum(txn_amount) as Total_Amount_Deposit
     from customer_transactions
     where txn_type = 'deposit'
     group by customer_id
     )
select avg(Deposit_Count) as AVG_Deposit_Count, avg(Total_Amount_Deposit) as AVG_Total_Amount_Deposit
from cte_counts;
